...
```
[赛道 ETF 触发条件，满足任一]
    ① 翻倍股策略 / 热门赛道信号：近 7 日该赛道 ≥3 只不同 ticker 被提及
...
| **翻倍股选股策略** | 网络总结（另见 `frameworks/hot-sector-method.md`）| 七大赛道 × 翻倍股筛选（2A/2B/2C）；风险纪律：止盈提醒/动态观望 | 想法生成 + 执行纪律 |

> **热门赛道方法论 vs 翻倍股选股策略** — 两者建仓风格（金字塔/不怕追高/免费股）**完全一致，互相印证**。核心分工：
> - 翻倍股选股策略：七大赛道发现 + 2A/2B/2C 筛选漏斗 + 组合级风险纪律（止盈提醒/动态观望）
...
| 场景 | 账户 | 翻倍股策略 | 热门赛道方法论 | BAIT | Moneyball | Asset Types | SEPA | /morning-check |
|------|------|:--------:|:------:|:----:|:---------:|:-----------:|:----:|:--------------:|
| 初步筛选新标的 | 通用 | ✅ 主（2A/2B/2C）| 赛道验证 | — | — | — | — | — |
| /ticker-scan 扫描赛道候选 | 通用 | ✅ 主 | — | — | — | — | — | — |
...
### 翻倍股选股策略 ↔ BAIT
- 翻倍股策略 2A 特征 "盈利拐点" = BAIT **B 层**（市场情绪滞后于运营数据）
- 翻倍股策略 2A 特征 "订单积压可见性" = BAIT **A 层**（分析性低效，模型低估积压价值）
- 翻倍股策略七大赛道选题 = 主题性 **I 层**（散户尚未发现的行业信息不对称）
- 翻倍股策略 2B 五大公式"技术壁垒高" = BAIT **T 层**（时间性低效：技术壁垒短期难复制）

### 翻倍股选股策略 ↔ 热门赛道方法论
- **建仓风格完全一致**：金字塔式、"不怕追高"、免费股策略 — 两框架互相印证
- **风险纪律互补**：
  - 热门赛道方法论：单只止损触发、抛物线顶信号、ETF MA50 赛道止损
  - 翻倍股策略：组合级止盈提醒（多只同涨时主动锁利）、动态观望（宏观过滤层）
- **论文验证要求**：翻倍股策略明确要求独立财报验证（不直接采信二手描述）
...
### SEPA ↔ 翻倍股选股策略 / 热门赛道方法论
- 两者金字塔建仓原则一致；差异在精度：SEPA 要求精确 Pivot 突破 + 量能，翻倍股策略入场精度要求更低、更注重趋势持续性
- 共识：仓位上涨 100% 先收回成本（自由股策略 = SEPA Phase 2 止损移至保本 + 卖半锁利）
- 差异：SEPA 的买入区 = Pivot +5%（有上限）；翻倍股策略 "不怕追高"（追入 = 确认信号，无硬上限）
...
1. **研究质量优先于信号速度**：策略信号是起点，不是终点。2A/2B/2C 三层筛选 + 独立财报验证是必须步骤。
...
- 翻倍股选股策略（网络总结）：七大赛道 + 2A/2B/2C + 风险纪律，本仓库见 `frameworks/hot-sector-method.md`
- 执行工具（Roth）：`/morning-check`（日常入场 + 批量扫描 + 三类型矩阵）、`/sector-check`（赛道 ETF 定投）、`/etf-analyze`（ETF 深度分析）、`/ticker-scan`（七大赛道候选扫描）
